...
long perft(Board* board_ptr, int depth) {
	MoveList move_list = {};
...
void run_perft_test(char* fen_string, long* expected_results, int max_depth) {
	Board board = {};
	setup_board(&board, fen_string);

	printf("%s\n", fen_string);
	for (int i = 0; i < max_depth; i++) {
		long found = perft(&board, i + 1);
		long expected = expected_results[i];
		printf((found == expected) ? "🟢 PASSED!\t": "🔴 FAILED!\t");
		printf("[%d] ", i + 1);
		printf("found: %ld ", found);
		printf("expected: %ld\n", expected);
	}
	printf("\n");
}
...
void run_perft_suite() {
	run_perft_test(
		...
		(long [8]) {20, 400, 8902, 197281, 4865609, 119060324, 3195901860, 84998978956},
		4
	);

	run_perft_test(
		"r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",
		(long [6]) {48, 2039, 97862, 4085603, 193690690, 8031647685},
		4
	);

	run_perft_test(
		"8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1",
		(long [8]) {14, 191, 2812, 43238, 674624, 11030083, 178633661, 3009794393},
		4
	);

	run_perft_test(
		"r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1",
		(long [6]) {6, 264, 9467, 422333, 15833292, 706045033},
		4
	);

	run_perft_test(
		"rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8",
		(long [5]) {44, 1486, 62379, 2103487, 89941194},
		4
	);

	run_perft_test(
		"r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10",
		(long [6]) {46, 2079, 89890, 3894594, 164075551, 6923051137},
		4
	);
}
